...
/// Directories that are never program-under-audit code: build output, VCS,
/// autogenerated client bindings, JS deps. Always excluded.
fn is_excluded_path(path: &Path) -> bool {
    path.components().any(|component| {
        let part = component.as_os_str().to_string_lossy();
        matches!(
            part.as_ref(),
            "target" | ".git" | "node_modules" | "generated" | "dist" | ".cargo"
        )
    })
}

/// Non-program directories excluded by default but includable with
/// `--include-tests`: test harnesses, benchmarks, and example/client code
/// (all of which routinely trip rules with scaffolding that never ships).
///
/// Vendored dependency crates (e.g. a program that ships `crates/pinocchio`
/// alongside its own source) are intentionally NOT auto-excluded here — the
/// heuristics for distinguishing a vendored SDK from a workspace member are
/// fragile. Exclude them per-project via `exclude` in `sentio.toml` instead.
fn is_test_path(path: &Path) -> bool {
    path.components().any(|component| {
        let part = component.as_os_str().to_string_lossy();
        matches!(
            part.as_ref(),
            "tests" | "test" | "fixtures" | "bench" | "benches" | "example" | "examples"
        )
    })
}
